...
def compare_headerlist_header(header_list, header):
    ...
    for entry in header_list:
        ...
        if 'comment' in entry:
            assert entry['comment'].strip() == header.get_comment(name).strip(), (
...
def compare_object_array(arr1, arr2, name, rows=None):
    ...
    for i, row in enumerate(rows):
        if (sys.version_info >= (3, 0, 0) and isinstance(arr2[i], bytes)) or isinstance(
            arr2[i], str
        ):
            ...
            orig = arr1[row]
            s = len(orig)
            compare_array(orig, delement[0:s], '%s num el %d equal' % (name, i))
...
def compare_rec_with_var(rec1, rec2, name, rows=None):
    ...
    if rows is None:
        ...
        assert rec1.size == rec2.size, "testing '%s' same number of rows" % name
    ...
    for f in rec2.dtype.names:
        ...
        if util.is_object(rec1[f]):
            compare_object_array(
                rec1[f], rec2[f], "testing '%s' field '%s'" % (name, f), rows=rows
            )
        else:
            compare_array(
                rec1[f][rows], rec2[f], "testing '%s' num field '%s' equal" % (name, f)
            )
