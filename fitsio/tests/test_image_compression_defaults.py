...
def test_compression_efns_kwargs():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        img = np.ones((20, 20))
        with fitsio.FITS(fn + '[compress G 5 10; qz 8.0]', 'rw', clobber=True) as fits:
...
def test_compression_hcomp_args():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        img = np.ones((20, 20))
        with fitsio.FITS(fn + '[compress HS 10 10; s 2.0]', 'rw', clobber=True) as fits:
            fits.write(img, dither_seed=42)
        hdr = fitsio.read_header(fn, ext=1)
        for key, val in [
            ('ZTILE1', 10),
...
def test_compression_multihdu_diskfile():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with fitsio.FITS(fn, 'rw', clobber=True) as fits:
            ...
            # C
            fits.write(
                img, extname='C', compress='GZIP', qmethod='SUBTRACTIVE_DITHER_2'
            )
            # D
