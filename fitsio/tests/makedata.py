...
def make_data():
    ...
    if sys.version_info > (3, 0, 0):
        dtype += [("Uscalar", Udtype), ("Uvec", Udtype, nvec), ("Uarr", Udtype, ashape)]
    ...
    vardata["Sscalar"] = ["%-6s" % s for s in ["hello", "world", "good", "bye"]]
    ...
    if sys.version_info >= (3, 0, 0):
        vardata["Uscalar"] = ["%-6s" % s for s in ["hello", "world", "good", "bye"]]
