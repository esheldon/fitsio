...
static PyObject *PyFITSObject_create_image_hdu(struct PyFITSObject *self,
    ...
    ALLOW_NOGIL;
    int ndims = 0;
    long dims[CFITSIO_MAX_ARRAY_DIMS] = {0};
    ...
    if (array_obj == Py_None) {
        if (NOGIL(create_empty_hdu(self, &status))) {
    ...
    if (write_data) {
        if (any_nan) {
            ...
            if (fits_write_imgnull(self->fits, datatype, firstpixel, nelements,
                ...
                goto create_image_hdu_gil_cleanup;
            }
        } else {
...
static PyObject *PyFITSObject_write_long_long_key(struct PyFITSObject *self,
    ...
    if (fits_movabs_hdu(self->fits, hdunum, &hdutype, &status)) {
        goto write_long_long_key_cleanup;
    }
...
static PyObject *PyFITSObject_write_logical_key(struct PyFITSObject *self,
    ...
    if (fits_movabs_hdu(self->fits, hdunum, &hdutype, &status)) {
        goto write_logical_key_cleanup;
    }
...
static PyObject *PyFITSObject_write_undefined_key(struct PyFITSObject *self,
    ...
    if (fits_movabs_hdu(self->fits, hdunum, &hdutype, &status)) {
        goto write_undefined_key_cleanup;
    }
...
static PyObject *PyFITSObject_read_var_column_as_list(struct PyFITSObject *self,
    ...
    for (i = 0; i < nrows; i++) {
        ...
        if (PyList_Append(listObj, tempObj)) {
            goto read_var_column_cleanup;
        }
        Py_XDECREF(tempObj);
        tempObj = NULL;
...
static PyObject *PyFITSObject_read_image(struct PyFITSObject *self,
    ...
    npy_dtype = PyArray_TYPE(array);
    if (npy_to_fits_image_types(npy_dtype, &dummy, &fits_read_dtype)) {
        UNLOCK_FITS(self);
        return NULL;
    }
...
static PyObject *PyFITSObject_read_header(struct PyFITSObject *self,
    ...
    for (i = 0; i < nkeys; i++) {
        ...
        if (add_string_to_dict(dict, "card_string", card)) {
            free(longstr);
            Py_XDECREF(dict);
            Py_XDECREF(list);
            UNLOCK_FITS(self);
        ...
        if (is_blank_key) {
            ...
            if (add_string_to_dict(dict, "value", "")) {
                free(longstr);
                Py_XDECREF(dict);
                Py_XDECREF(list);
                UNLOCK_FITS(self);
            ...
            if (add_string_to_dict(dict, "comment", scomment)) {
                free(longstr);
                Py_XDECREF(dict);
                Py_XDECREF(list);
                UNLOCK_FITS(self);
            ...
            // comment or history
            convert_to_ascii(scomment);
            if (add_string_to_dict(dict, "name", keyname)) {
                free(longstr);
                Py_XDECREF(dict);
                Py_XDECREF(list);
                UNLOCK_FITS(self);
            ...
            if (add_string_to_dict(dict, "value", scomment)) {
                free(longstr);
                Py_XDECREF(dict);
                Py_XDECREF(list);
                UNLOCK_FITS(self);
            ...
            if (add_string_to_dict(dict, "comment", comment)) {
                free(longstr);
                Py_XDECREF(dict);
                Py_XDECREF(list);
                UNLOCK_FITS(self);
            ...
            if (!is_string_value && 0 == strlen(longstr)) {
                ...
                if (add_none_to_dict(dict, "value")) {
                    free(longstr);
                    Py_XDECREF(dict);
                    Py_XDECREF(list);
                    UNLOCK_FITS(self);
                ...
                if (is_string_value) {
                    ...
                    if (add_string_to_dict(dict, "value", longstr)) {
                        free(longstr);
                        Py_XDECREF(dict);
                        Py_XDECREF(list);
                        UNLOCK_FITS(self);
                    ...
                    if (add_true_to_dict(dict, "value")) {
                        free(longstr);
                        Py_XDECREF(dict);
                        Py_XDECREF(list);
                        UNLOCK_FITS(self);
                    ...
                    if (add_false_to_dict(dict, "value")) {
                        free(longstr);
                        Py_XDECREF(dict);
                        Py_XDECREF(list);
                        UNLOCK_FITS(self);
                    ...
                    fits_read_key(self->fits, TDOUBLE, keyname, &dval, comment,
                                  &status);
                    if (add_double_to_dict(dict, "value", dval)) {
                        free(longstr);
                        Py_XDECREF(dict);
                        Py_XDECREF(list);
                        UNLOCK_FITS(self);
                    ...
                    if (fits_read_key(self->fits, TLONGLONG, keyname, &lval,
                        ...
                        if (add_long_long_to_dict(dict, "value",
                                                  (long long)lval)) {
                            free(longstr);
                            Py_XDECREF(dict);
                            Py_XDECREF(list);
...
write_checksum_cleanup:
    ...
    // PyDict_New will set an error internally if it returns NULL, so
    // we can simply return it.
    dict = PyDict_New();
    if (dict != NULL) {
        if (add_long_long_to_dict(dict, "datasum", (long long)datasum)) {
            Py_XDECREF(dict);
            UNLOCK_FITS(self);
            return NULL;
        }
        if (add_long_long_to_dict(dict, "hdusum", (long long)hdusum)) {
            Py_XDECREF(dict);
            UNLOCK_FITS(self);
            return NULL;
        }
...
verify_checksum_cleanup:
    ...
    dict = PyDict_New();
    if (dict != NULL) {
        if (add_long_to_dict(dict, "dataok", (long)dataok)) {
            Py_XDECREF(dict);
            UNLOCK_FITS(self);
            return NULL;
        }
        if (add_long_to_dict(dict, "hduok", (long)hduok)) {
            Py_XDECREF(dict);
            UNLOCK_FITS(self);
            return NULL;
        }
...
parse_card_cleanup:
    ...
    // Py_BuildValue sets an error internally and returns NULL if it fails
    // so no null checking here
    if (is_undefined) {
