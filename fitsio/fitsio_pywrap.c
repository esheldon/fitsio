...
static PyObject *PyFITSObject_close(struct PyFITSObject *self) {
    ...
    static npy_int64 *get_int64_from_array(PyArrayObject * arr,
                                           npy_intp * ncols) {
        ...
        check = (npy_type == NPY_INT64) |
                (npy_type == NPY_LONGLONG &&
                 sizeof(npy_longlong) == sizeof(npy_int64));
        if (!check) {
        ...
        if (!PyArray_ISCONTIGUOUS(arr)) {
            PyErr_SetString(PyExc_TypeError,
                            "int64 array must be a contiguous.");
            return NULL;
        }
    ...
    static PyObject *PyFITSObject_movnam_hdu(struct PyFITSObject * self,
        ...
        if (!PyArg_ParseTuple(args, (char *)"isi", &hdutype, &extname,
                              &extver)) {
            return NULL;
        }
    ...
    static PyObject *PyFITSObject_get_hdu_info(struct PyFITSObject * self,
        ...
        long long header_start = -1;
        long long data_start = -1;
        long long data_end = -1;
        ...
        int bitpix_equiv = 0;
        char comptype[20] = "";
        PyObject *dimsObj = NULL;
        int i;
        int img_error = 0;
        int comptype_status = 0;

        LONGLONG nrows = 0;
        int ncols = 0;
        PyObject *colinfo = NULL;
        int j;
        ...
        tstatus = 0;
        if (fits_file_name(self->fits, filename, &tstatus) == 0) {
        ...
        tstatus = 0;
        fits_read_key(self->fits, TINT, "EXTVER", &extver, NULL, &tstatus);

        tstatus = 0;
        fits_read_key(self->fits, TINT, "HDUVER", &hduver, NULL, &tstatus);

        tstatus = 0;
        is_compressed = fits_is_compressed_image(self->fits, &tstatus);

        // get byte offsets
        fits_get_hduaddrll(self->fits, &header_start, &data_start, &data_end,
                           &tstatus);

        if (hdutype == IMAGE_HDU) {
            // if (fits_read_imghdrll(self->fits, maxdim, simple_p, &bitpix,
            // &ndims,
            //                        dims, pcount_p, gcount_p, extend_p,
            //                        &status))
            //                        {
            tstatus = 0;
            if (fits_get_img_paramll(self->fits, maxdim, &bitpix, &ndims, dims,
                ...
                if (ignore_scaling == TRUE) {
                    ...
                    fits_get_img_type(self->fits, &bitpix_equiv, &tstatus);
                } else {
                    fits_get_img_equivtype(self->fits, &bitpix_equiv, &tstatus);
                }

                comptype_status = 0;
                if (fits_read_key(self->fits, TSTRING, "ZCMPTYPE", comptype,
                                  NULL, &comptype_status) == 0) {
        ...
        if (hdutype == IMAGE_HDU) {
            ...
            colinfo = PyList_New(0);
            {
                ...
                for (i = 0; i < ncols; i++) {
                    ...
                    tstatus = 0;
                    fits_get_coltypell(self->fits, i + 1, &type, &repeat,
                                       &width, &tstatus);
                    ...
                    tstatus = 0;
                    fits_get_eqcoltypell(self->fits, i + 1, &type, &repeat,
                                         &width, &tstatus);
                    add_long_to_dict(d, "eqtype", (long)type);

                    tstatus = 0;
                    if (fits_read_tdimll(self->fits, i + 1, maxdim, &ndims,
                                         dims, &tstatus)) {
                        add_none_to_dict(d, "tdim");
                    } else {
                        PyObject *dimsObj = PyList_New(0);
                        for (j = 0; j < ndims; j++) {
                            append_long_long_to_list(dimsObj,
                                                     (long long)dims[j]);
                        }
    ...
    // get info for the specified HDU
    static PyObject *PyFITSObject_get_hdu_name_version(
        struct PyFITSObject * self, PyObject * args) {
        ALLOW_NOGIL;
        int hdunum = 0, hdutype = 0;
        ...
        tstatus = 0;
        fits_read_key(self->fits, TSTRING, "EXTNAME", extname, NULL, &tstatus);
    ...
    /*
     * this is really only for reading variable length columns since we should
     * be able to just read the bytes for normal columns
     */
    ...
    /*
       ...
       It is useful to create the extension first so we can write keywords into
       the header before adding data.  This avoids moving the data if the header
       grows too large.
    ...
    static PyObject *PyFITSObject_create_image_hdu(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwds, "Oi|OOOOOOOOsii", kwlist, &array_obj, &nkeys,
                &dims_obj, &comptype_obj, &tile_dims_obj, &qlevel_obj,
                &qmethod_obj, &dither_seed_obj, &hcomp_scale_obj,
                &hcomp_smooth_obj, &extname, &extver, &any_nan)) {
            py_status = 1;
        } else {
            if (array_obj != Py_None) {
                ...
                if (comptype_obj != Py_None) {
                    ...
                    // get what the code is going to use just in case we
                    // override the HCOMPRESS options
        ...
        if (array_obj == Py_None) {
            ...
            if (comptype == HCOMPRESS_1) {
                if (hcomp_scale_obj != Py_None) {
                    if (fits_set_hcomp_scale(self->fits, hcomp_scale,
                                             &status)) {
                        goto create_image_hdu_cleanup;
                    }
                }
                ...
                if (hcomp_smooth_obj != Py_None) {
                    if (fits_set_hcomp_smooth(self->fits, hcomp_smooth,
                                              &status)) {
                        goto create_image_hdu_cleanup;
                    }
                }
            ...
            if (fits_create_img(self->fits, image_datatype, ndims, dims,
                                &status)) {
                goto create_image_hdu_cleanup;
            } else {
                flush_hdu = 1;
        ...
        if (extname != NULL) {
            if (strlen(extname) > 0) {
                ...
                if (extver > 0) {
                    if (fits_update_key_lng(self->fits, "EXTVER",
        ...
        if (write_data) {
            if (any_nan) {
                ...
                fits_write_img(self->fits, datatype, firstpixel, nelements,
                               data, &status);
            }
            if (status != 0) {
                goto create_image_hdu_cleanup;
    ...
    create_image_hdu_cleanup:
        ...
        if (got_dither_seed == 1) {
            ...
            fits_set_dither_seed(self->fits, orig_dither_seed,
                                 &dither_seed_status);
        }
    ...
    static PyObject *PyFITSObject_write_subset(struct PyFITSObject * self,
        ...
        if (!PyArray_Check(firstpixel)) {
            PyErr_SetString(PyExc_TypeError,
                            "input firstpixel must be an array.");
            return NULL;
        }
        ...
        if (!PyArray_Check(lastpixel)) {
            PyErr_SetString(PyExc_TypeError,
                            "input lastpixel must be an array.");
            return NULL;
        }
        ...
        if (fits_movabs_hdu(self->fits, hdunum, &hdutype, &status)) {
            goto write_image_subset_cleanup;
        }
     ...
     */
    static int add_tdims_from_listobj(struct PyFITSObject * self,
                                      fitsfile * fits, PyObject * tdimObj,
                                      int ncols) {
        int status = 0, i = 0;
        size_t size = 0;
        char keyname[20];
    ...
    static PyObject *PyFITSObject_create_table_hdu(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwds, "iiOO|OOsi", kwlist, &table_type, &nkeys, &ttypObj,
                &tformObj, &tunitObj, &tdimObj, &extname, &extver)) {
            return NULL;
        }
    ...
    static PyObject *PyFITSObject_insert_col(struct PyFITSObject * self,
        ...
        static char *kwlist[] = {"hdunum", "colnum", "ttyp",
                                 "tform",  "tdim",   NULL};
        // these are all strings
    ...
    static int write_string_column(
        ...
        if (hdutype == ASCII_TBL || CFITSIO_MAJOR < 4) {
            LONGLONG i = 0;
            LONGLONG twidth = 0;
            // need to create a char** representation of the data, just point
            // back into the data array at string width offsets.  the
            // fits_write_col_str takes care of skipping between fields.
            char *cdata = NULL;
            char **strdata = NULL;
    ...
    // write a column, starting at firstrow.  On the python side, the firstrow
    // kwd should default to 1. You can append rows using firstrow = nrows+1
    /*
    static PyObject *
    PyFITSObject_write_column(struct PyFITSObject* self, PyObject* args,
    PyObject* kwds) { int status=0; int hdunum=0; int hdutype=0; int colnum=0;
    int write_bitcols=0; PyObject* array=NULL;
    ...
    columns"); return NULL;
        ...
        if (fits_dtype == TSTRING) {
            ...
            if (write_string_column(self, self->fits, colnum, firstrow,
    firstelem, nelem, data, &status)) { set_ioerr_string_from_status(status,
    self); return NULL;
            }
        } else if (fits_dtype == TBIT) {
            if (fits_write_col_bit(self->fits, colnum, firstrow, firstelem,
    nelem, data, &status)) { set_ioerr_string_from_status(status, self); return
    NULL;
            }
        } else {
            if( fits_write_col(self->fits, fits_dtype, colnum, firstrow,
    firstelem, nelem, data, &status)) { set_ioerr_string_from_status(status,
    self); return NULL;
            }
        }
    ...
    */
    ...
    static PyObject *PyFITSObject_write_columns(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        int hdunum = 0;
        int hdutype = 0;
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwds, "iOOLi", kwlist, &hdunum,
                                         &colnum_list, &array_list,
                                         &firstrow_py, &write_bitcols)) {
            return NULL;
        }
        firstrow = (LONGLONG)firstrow_py;
        ...
        for (icol = 0; icol < ncols; icol++) {
            ...
            fits_dtypes[icol] =
                npy_to_fits_table_type(npy_dtype, write_bitcols);
            ...
            if (fits_dtypes[icol] == TLOGICAL) {
                ...
                // if the column exists and is declared TBIT we will write
                // that way instead
                if (fits_get_coltypell(self->fits, colnums[icol], &ttype,
        ...
        for (irow = 0; irow < nelem; irow++) {
            ...
            for (icol = 0; icol < ncols; icol++) {
                ...
                if (is_string[icol]) {
                    if (write_string_column(
                            self, self->fits, hdutype, colnums[icol], thisrow,
                            firstelem, nperrow[icol], (char *)data, &status)) {
                        set_ioerr_string_from_status(status, self);
                        goto _fitsio_pywrap_write_columns_bail;
                    }
                    ...
                    if (fits_write_col(self->fits, fits_dtypes[icol],
    ...
    static int write_var_string_column(
        ...
        for (i = 0; i < nrows; i++) {
            ...
            // just a container
            strarr[0] = strdata;
            res = fits_write_col_str(fits, colnum, firstrow + i, firstelem,
                                     nelem, strarr, status);
    ...
    static int write_var_num_column(
        ...
        int colnum,        /* I - number of column to write (1 = 1st col) */
        LONGLONG firstrow, /* I - first row to write (1 = 1st row)        */
        int fits_dtype, PyArrayObject *array,
    ...
    /*
     * write a variable length column, starting at firstrow.  On the python
     * side, the firstrow kwd should default to 1.  You can append rows using
     * firstrow = nrows+1
     *
    ...
    static PyObject *PyFITSObject_write_var_column(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        int hdunum = 0;
        int hdutype = 0;
        ...
        // determine the fits dtype for this column.  We will use this to get
        // data from the array for writing
        ...
        if (fits_dtype == -TSTRING) {
            ...
            if (write_var_num_column(self, self->fits, colnum, firstrow,
                                     fits_dtype, array, &status)) {
                set_ioerr_string_from_status(status, self);
                return NULL;
            }
        }
    ...
    */
    ...
    static PyObject *PyFITSObject_write_string_key(struct PyFITSObject * self,
        ...
        if (fits_update_key_longstr(self->fits, keyname, value, comment,
                                    &status)) {
            set_ioerr_string_from_status(status, self);
            return NULL;
        }
    ...
    static PyObject *PyFITSObject_write_long_long_key(
        struct PyFITSObject * self, PyObject * args) {
        int status = 0;
        int hdunum = 0;
        int hdutype = 0;
    ...
    */
    ...
    static PyObject *PyFITSObject_write_undefined_key(
        struct PyFITSObject * self, PyObject * args) {
        int status = 0;
        int hdunum = 0;
        int hdutype = 0;
    ...
    */
    ...
    static PyObject *PyFITSObject_insert_rows(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        int hdunum = 0;
    ...
    */
    ...
    static PyObject *PyFITSObject_delete_row_range(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        int hdunum = 0;
    ...
    */
    ...
    static PyObject *PyFITSObject_delete_rows(
        struct PyFITSObject * self, PyObject * args, PyObject * kwds) {
        int status = 0;
        int hdunum = 0;
    ...
    /*
     * read a single, entire column from an ascii table into the input array.
     * This version uses the standard read column instead of our by-bytes
     * version.
     *
    ...
    static int read_ascii_column_all(fitsfile * fits, int colnum,
        ...
        if (fits_dtype == TSTRING) {
            ...
            for (i = 0; i < nelem; i++) {
                ...
                rownum = (LONGLONG)(1 + i);
                if (fits_read_col_str(fits, colnum, rownum, firstelem, 1,
            ...
            for (i=0; i<nelem; i++) {
                //strdata[i] = &cdata[twidth*i];
                // this 1-d assumption works because array fields are not
            allowedin ascii strdata[i] = (char*) PyArray_GETPTR1(array, i);
            }
            ...
            */
            ...
            data = PyArray_DATA(array);
            if (fits_read_col(fits, fits_dtype, colnum, firstrow, firstelem,
    ...
    static int read_ascii_column_byrow(
        fitsfile * fits, int colnum, PyArrayObject *array, PyArrayObject *rows,
        ...
        if ((PyObject *)rows != Py_None) {
            ...
            if (nrows != nelem) {
                PyErr_Format(
                    PyExc_ValueError,
                    "input array[%ld] and rows[%ld] have different size", nelem,
                    nrows);
                return 1;
            }
        }
        ...
        for (i = 0; i < nrows; i++) {
            ...
            if (fits_dtype == TSTRING) {
                ...
                if (fits_read_col(fits, fits_dtype, colnum, rownum, firstelem,
    ...
    static int read_ascii_column(fitsfile * fits, int colnum,
                                 PyArrayObject *array, PyArrayObject *rows,
                                 PyArrayObject *sortind, int *status) {

        int ret = 0;
        if ((PyObject *)rows != Py_None || !PyArray_ISCONTIGUOUS(array)) {
            ret = read_ascii_column_byrow(fits, colnum, array, rows, sortind,
                                          status);
        } else {
    ...
    static int read_binary_column(fitsfile * fits, int colnum, npy_intp nrows,
                                  ...
                                  void *vdata, npy_intp stride, int *status) {
    ...
    static PyObject *PyFITSObject_read_column(struct PyFITSObject * self,
        ...
        if (!PyArg_ParseTuple(args, (char *)"iiOOO", &hdunum, &colnum,
        ...
        if (hdutype == ASCII_TBL) {
            ...
            if (read_binary_column(self->fits, colnum, nrows, rows, sortind,
                                   data, stride, &status)) {
                set_ioerr_string_from_status(status, self);
                return NULL;
            }
    ...
    static PyObject *read_var_nums(fitsfile * fits, int colnum, LONGLONG row,
                                   LONGLONG nelem, int fits_dtype,
                                   int npy_dtype, int *status) {
        LONGLONG firstelem = 1;
        PyArrayObject *array = NULL;
        void *nulval = 0;
        int *anynul = NULL;
        ...
        if (array == NULL) {
            PyErr_Format(PyExc_MemoryError,
                         "Could not allocate array type %d size %lld",
                         npy_dtype, nelem);
            return NULL;
        }
        data = PyArray_DATA(array);
     ...
     * what about strings?
     */
    static PyObject *PyFITSObject_read_var_column_as_list(
        struct PyFITSObject * self, PyObject * args) {
        int hdunum = 0;
        int colnum = 0;
        ...
        if (rows_obj == Py_None) {
            ...
            sortind = (const npy_int64 *)get_int64_from_array(sortind_array,
                                                              &nsortind);
        ...
        for (i = 0; i < nrows; i++) {
            ...
            if (fits_dtype == -TSTRING) {
                tempObj =
                    read_var_string(self->fits, colnum, row, repeat, &status);
            } else {
    ...
    static int read_binary_rec_columns(fitsfile * fits, npy_intp ncols,
        ...
        for (irow = 0; irow < nrows; irow++) {
            ...
            for (col = 0; col < ncols; col++) {
                ...
                file_pos =
                    hdu->datastart + row * hdu->rowlength + colptr->tbcol;
                ...
                if (colptr->tdatatype == TBIT) {
                    ...
                    ffmbyt(fits, file_pos, REPORT_EOF, status);
                    if (ffgbytoff(fits, width, repeat, 0, (void *)ptr,
                                  status)) {
                        return 1;
                    }
                }
    ...
    static PyObject *PyFITSObject_read_columns_as_rec(
        struct PyFITSObject * self, PyObject * args) {
        int hdunum = 0;
        int hdutype = 0;
        npy_intp ncols = 0;
        ...
        if (rows_obj == Py_None) {
            ...
            sortind =
                get_int64_from_array((PyArrayObject *)sortind_obj, &nsortind);
            if (sortind == NULL) {
                return NULL;
            }
        }
        if (read_binary_rec_columns(self->fits, ncols, colnums, nrows, rows,
                                    sortind, (PyArrayObject *)array_obj,
                                    &status)) {
            goto recread_columns_cleanup;
        }
    ...
    static int read_columns_as_rec_byoffset(
        ...
        npy_intp nrows, const npy_int64 *rows, const npy_int64 *sortind,
        char *data, npy_intp recsize, int *status) {
     ...
     */
    static PyObject *PyFITSObject_read_columns_as_rec_byoffset(
        struct PyFITSObject * self, PyObject * args) {
        int status = 0;
        int hdunum = 0;
        int hdutype = 0;
        ...
        if (!PyArg_ParseTuple(args, (char *)"iOOOOO", &hdunum, &columns_obj,
                              &offsets_obj, &array_obj, &rows_obj,
                              &sortind_obj)) {
            return NULL;
        }
        ...
        if (read_columns_as_rec_byoffset(self->fits, ncols, colnums, offsets,
                                         nrows, rows, sortind, (char *)data,
                                         recsize, &status) > 0) {
            goto recread_columns_byoffset_cleanup;
        }
    ...
    static int read_rec_bytes_byrow(fitsfile * fits, npy_intp nrows,
                                    npy_int64 * rows, npy_int64 * sortind,
        ...
        for (irow = 0; irow < nrows; irow++) {
            ...
            if (fits_read_tblbytes(fits, firstrow, firstchar, hdu->rowlength,
                                   ptr, status)) {
                return 1;
            }
    ...
    */
    ...
    static PyObject *PyFITSObject_read_rows_as_rec(struct PyFITSObject * self,
        ...
        if (read_rec_bytes_byrow(self->fits, nrows, rows, sortind, data,
                                 &status)) {
            goto recread_byrow_cleanup;
        }
    ...
    static int read_rec_range(fitsfile * fits, LONGLONG firstrow,
                              LONGLONG nrows, void *data, int *status) {
    ...
    static PyObject *PyFITSObject_read_as_rec(struct PyFITSObject * self,
        ...
        if (!PyArg_ParseTuple(args, (char *)"iLLO", &hdunum, &firstrow,
                              &lastrow, &array_obj)) {
            return NULL;
        }
        ...
        if (read_rec_range(self->fits, (LONGLONG)firstrow, (LONGLONG)nrows,
                           data, &status)) {
            goto recread_asrec_cleanup;
        }
    ...
    // read an n-dimensional "image" into the input array.  Only minimal
    // checking of the input array is done. Note numpy allows a maximum of 32
    // dimensions
    ...
    static int get_long_slices(
        PyArrayObject * fpix_arr, PyArrayObject * lpix_arr,
        PyArrayObject * step_arr, long **fpix, long **lpix, long **step) {
    ...
    // read an n-dimensional "image" into the input array.  Only minimal
    // checking of the input array is done.
    static PyObject *PyFITSObject_read_image_slice(struct PyFITSObject * self,
        ...
        if (!PyArg_ParseTuple(args, (char *)"iOOOiO", &hdunum, &fpix_obj,
                              &lpix_obj, &step_obj, &ignore_scaling, &array)) {
            return NULL;
        }

        if (get_long_slices((PyArrayObject *)fpix_obj,
        ...
        for (i = 0; i < nkeys; i++) {
            ...
            if (ls == 0) {
                ...
                if (strncmp(keyname, "COMMENT", tocomp) == 0 ||
                    ...
                    if (strncmp(card, "HIERARCH", 8) == 0) {
                        ...
                        has_quote = 0;
                        // Look for 0 or more space characters followed by a
                        // quote character.
            ...
            if (is_blank_key) {
                ...
                if (is_hierarch) {
                    ...
                    // except
                    // *, ? and #. Thus we convert any of those (and any chars
                    // we find that don't correspond to something written, ascii
                    // <= 32 or 127) to an underscore if the key is converted,
                    // then we cannot parse it further with cfitsio
                    was_converted =
                        convert_keyword_to_allowed_ascii_template_and_nonascii_only(
                            keyname);
                } else {
                    // for non-hierach keys, we cannot use the cfitsio functions
                    // if any character besides those in the fits conventions
                    // (A-Z,a-z,0-9,_,-) are present. Thus we flag those and
                    // store their values as a string if this happens.
                    was_converted = has_invalid_keyword_chars(keyname);

                    // in order to actually store the key in the python dict, we
                    // have to cut out any non-ascii chars we additionally
                    // convert the template chars to that the fits data we make
                    // can be written back without error note that the check by
                    // has_invalid_keyword_chars is more stringent than the
                    // checks done here, so if any conversion is done it has
                    // already been flagged above.
                    convert_keyword_to_allowed_ascii_template_and_nonascii_only(
                        keyname);
                }
                add_string_to_dict(dict, "name", keyname);
                ...
                if (!is_string_value && 0 == strlen(longstr)) {
                    ...
                    if (is_string_value) {
                        ...
                        // if we had to convert bad characters in the keyword
                        // name we can't attempt to get a numerical value using
                        // fits_read_key because some characters in a keyword
                        // name cause a seg fault
                        convert_to_ascii(longstr);
                        add_string_to_dict(dict, "value", longstr);
                        ...
                        if (fits_read_key(self->fits, TLONGLONG, keyname, &lval,
                            ...
                            add_long_long_to_dict(dict, "value",
                                                  (long long)lval);
                        }
                    }
                }
    ...
    static PyObject *PyFITSObject_where(struct PyFITSObject * self,
                                        PyObject * args) {
        ALLOW_NOGIL;
        int status = 0;
        int hdunum = 0;
    ...
    where_function_cleanup:
        ...
        if (indices_obj == NULL) {
            PyErr_SetString(PyExc_MemoryError,
                            "Could not allocate index array");
            return NULL;
        } else if (status != 0) {
            set_ioerr_string_from_status(status, self);
            return NULL;
    ...
    */
    ...
    /*
        ...
        note the special first four comment fields will not be called comment
       but structural!  That will cause an exception to be raised, so the card
    ...
    */
    ...
    static PyObject *PyFITS_parse_card(PyObject * self, PyObject * args) {
        ...
        // only proceed if not comment or history, but note the special first
        // four comment fields will not be called comment but structural!  That
        // will cause an exception to be raised, so the card should be checked
    ...
    parse_card_cleanup:
        ...
        if (is_undefined) {
            ...
            output =
                Py_BuildValue("issss", keyclass, name, value, dtype, comment);
        }
        return output;
    }
    ...
    static PyMethodDef PyFITSObject_methods[] = {
        ...
        {"write_checksum", (PyCFunction)PyFITSObject_write_checksum,
         METH_VARARGS,
         "write_checksum\n\nCompute and write the checksums into the header."},
        {"verify_checksum", (PyCFunction)PyFITSObject_verify_checksum,
         METH_VARARGS,
        ...
        {"write_long_long_key", (PyCFunction)PyFITSObject_write_long_long_key,
         METH_VARARGS,
         "write_long_long_key\n\nWrite a long long key into the specified "
         "HDU."},
        ...
        {"write_comment", (PyCFunction)PyFITSObject_write_comment, METH_VARARGS,
         "write_comment\n\nWrite a comment into the header of the specified "
         "HDU."},
        {"write_history", (PyCFunction)PyFITSObject_write_history, METH_VARARGS,
         "write_history\n\nWrite history into the header of the specified "
         "HDU."},
        {"write_continue", (PyCFunction)PyFITSObject_write_continue,
         METH_VARARGS,
         "write_continue\n\nWrite contineu into the header of the specified "
         "HDU."},
    ...
    static PyMethodDef fitstype_methods[] = {
        ...
        {"cfitsio_is_bundled", (PyCFunction)PyFITS_cfitsio_is_bundled,
         METH_NOARGS,
         "cfitsio_is_bundled\n\nReturn True if library was built with a "
         "bundled "
         "copy of cfitsio."},
        {"cfitsio_has_bzip2_support",
         (PyCFunction)PyFITS_cfitsio_has_bzip2_support, METH_NOARGS,
         "cfitsio_has_bzip2_support\n\nReturn True if cfitsio has support for "
         "bzip2."},
        {"cfitsio_has_curl_support",
         (PyCFunction)PyFITS_cfitsio_has_curl_support, METH_NOARGS,
         "cfitsio_has_curl_support\n\nReturn True if cfitsio has support for "
         "curl."},
        ...
        {"get_key_meta", (PyCFunction)PyFITS_get_key_meta, METH_VARARGS,
         "get_key_meta\n\nparse the card to get key metadata "
         "(keyclass,dtype)."},
        {NULL} /* Sentinel */
    };
