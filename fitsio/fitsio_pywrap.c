...
static int has_invalid_keyword_chars(char *str) {
  ...
  for (i = 0; i < size; i++) {
    ...
    isbad = !((cval >= 'A' && cval <= 'Z') || (cval >= 'a' && cval <= 'z') ||
...
static npy_int64 *get_int64_from_array(PyArrayObject *arr, npy_intp *ncols) {
  ...
  check = (npy_type == NPY_INT64) | (npy_type == NPY_LONGLONG &&
                                     sizeof(npy_longlong) == sizeof(npy_int64));
  if (!check) {
    PyErr_Format(PyExc_TypeError, "array must be an int64 array (%d), got %d.",
                 NPY_INT64, npy_type);
    return NULL;
  }
...
static PyObject *PyFITSObject_get_hdu_info(struct PyFITSObject *self,
  ...
  tstatus = 0;
  if (fits_read_key(self->fits, TSTRING, "EXTNAME", extname, NULL, &tstatus) ==
      0) {
  ...
  tstatus = 0;
  if (fits_read_key(self->fits, TSTRING, "HDUNAME", hduname, NULL, &tstatus) ==
      0) {
  ...
  // get byte offsets
  if (0 == fits_get_hduaddrll(self->fits, &header_start, &data_start, &data_end,
                              &tstatus)) {
...
#else
  case NPY_UINT64:
    PyErr_SetString(PyExc_TypeError, "Unsigned 8 byte integer images are not "
  ...
  if (tile_dims_obj != Py_None) {
    ...
    if (tile_dims_py == NULL) {
      *status = 1;
      ...
      if (!tile_dims_fits) {
        PyErr_Format(PyExc_MemoryError, "failed to allocate %ld longs", ndims);
        goto _set_compression_bail;
      }
...
static PyObject *PyFITSObject_create_image_hdu(struct PyFITSObject *self,
  ...
  if (extname != NULL) {
    if (strlen(extname) > 0) {
      ...
      if (extver > 0) {
        if (fits_update_key_lng(self->fits, "EXTVER", (LONGLONG)extver, NULL,
                                &status)) {
          set_ioerr_string_from_status(status, self);
          goto create_image_hdu_cleanup;
        }
...
static PyObject *PyFITSObject_reshape_image(struct PyFITSObject *self,
  ...
  if (fits_get_img_paramll(self->fits, maxdim, &bitpix, &ndims_orig, dims_orig,
                           &status)) {
    set_ioerr_string_from_status(status, self);
    return NULL;
  }
...
static int add_tdims_from_listobj(struct PyFITSObject *self, fitsfile *fits,
  ...
  if (size != (size_t)ncols) {
    PyErr_Format(PyExc_ValueError,
                 "Expected %d elements in tdims list, got %ld", ncols, size);
    return 1;
  }
...
static PyObject *PyFITSObject_create_table_hdu(struct PyFITSObject *self,
  ...
  static char *kwlist[] = {"table_type", "nkeys",   "ttyp",   "tform", "tunit",
                           "tdim",       "extname", "extver", NULL};
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "iiOO|OOsi", kwlist, &table_type,
                                   &nkeys, &ttypObj, &tformObj, &tunitObj,
                                   &tdimObj, &extname, &extver)) {
    return NULL;
  }
...
static PyObject *PyFITSObject_write_columns(struct PyFITSObject *self,
  ...
  for (irow = 0; irow < nelem; irow++) {
    ...
    for (icol = 0; icol < ncols; icol++) {
      ...
      if (is_string[icol]) {
        if (write_string_column(self, self->fits, colnums[icol], thisrow,
                                firstelem, nperrow[icol], (char *)data,
                                &status)) {
          set_ioerr_string_from_status(status, self);
          goto _fitsio_pywrap_write_columns_bail;
        }
        ...
        if (fits_write_col_bit(self->fits, colnums[icol], thisrow, firstelem,
        ...
        if (fits_write_col(self->fits, fits_dtypes[icol], colnums[icol],
                           thisrow, firstelem, nperrow[icol], data, &status)) {
          set_ioerr_string_from_status(status, self);
          goto _fitsio_pywrap_write_columns_bail;
        }
      }
...
static int write_var_num_column(
  ...
  for (i = 0; i < nrows; i++) {
    ...
    el_array = (PyArrayObject *)PyArray_CheckFromAny(
        el, PyArray_DescrFromType(npy_dtype), mindepth, maxdepth, requirements,
        context);
    if (el_array == NULL) {
    ...
    res = fits_write_col(fits, abs(fits_dtype), colnum, firstrow + i, firstelem,
                         (LONGLONG)nelem, data, status);
    Py_XDECREF(el_array);
...
static PyObject *PyFITSObject_write_var_column(struct PyFITSObject *self,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "iiOL", kwlist, &hdunum, &colnum,
                                   &array_obj, &firstrow_py)) {
    return NULL;
  }
  firstrow = (LONGLONG)firstrow_py;
...
static int read_ascii_column_all(fitsfile *fits, int colnum,
  ...
  if (fits_dtype == TSTRING) {
    ...
    for (i = 0; i < nelem; i++) {
      ...
      rownum = (LONGLONG)(1 + i);
      if (fits_read_col_str(fits, colnum, rownum, firstelem, 1, nulstr, &cdata,
                            anynul, status) > 0) {
        return 1;
      }
    }
    ...
    strdata=malloc(nelem*sizeof(char*));
    if (NULL==strdata) {
        PyErr_SetString(PyExc_MemoryError, "could not allocate temporary string
    pointers"); *status = 99; return 1;
...
static int read_ascii_column_byrow(fitsfile *fits, int colnum,
  ...
  for (i = 0; i < nrows; i++) {
    ...
    if (fits_dtype == TSTRING) {
      ...
      if (fits_read_col(fits, fits_dtype, colnum, rownum, firstelem, 1, nulval,
                        data, anynul, status) > 0) {
        return 1;
      }
    }
  }
...
static int read_ascii_column(fitsfile *fits, int colnum, PyArrayObject *array,
  ...
  if ((PyObject *)rows != Py_None || !PyArray_ISCONTIGUOUS(array)) {
    ret = read_ascii_column_byrow(fits, colnum, array, rows, sortind, status);
  } else {
...
static PyObject *PyFITSObject_read_column(struct PyFITSObject *self,
  ...
  if (hdutype == ASCII_TBL) {
    if (read_ascii_column(self->fits, colnum, array, rows_array, sortind_array,
                          &status)) {
      set_ioerr_string_from_status(status, self);
      return NULL;
    }
...
static PyObject *read_var_string(fitsfile *fits, int colnum, LONGLONG row,
  ...
  if (str == NULL) {
    PyErr_Format(PyExc_MemoryError, "Could not allocate string of size %lld",
                 nchar);
    return NULL;
  }
...
#endif
  if (NULL == stringObj) {
    PyErr_Format(PyExc_MemoryError, "Could not allocate py string of size %lld",
                 nchar);
    goto read_var_string_cleanup;
  }
...
static PyObject *PyFITSObject_read_var_column_as_list(struct PyFITSObject *self,
  ...
  if (!isvariable) {
    PyErr_Format(PyExc_TypeError, "Column %d not a variable length %d", colnum,
                 fits_dtype);
    return NULL;
  }
...
static PyObject *PyFITSObject_read_columns_as_rec(struct PyFITSObject *self,
  ...
  if (read_binary_rec_columns(self->fits, ncols, colnums, nrows, rows, sortind,
                              (PyArrayObject *)array_obj, &status)) {
    goto recread_columns_cleanup;
  }
...
PyFITSObject_read_columns_as_rec_byoffset(struct PyFITSObject *self,
  ...
  if (noffsets != ncols) {
    PyErr_Format(PyExc_ValueError, "%ld columns requested but got %ld offsets",
                 ncols, noffsets);
    return NULL;
  }

  if (rows_obj != Py_None) {
    rows = (const npy_int64 *)get_int64_from_array((PyArrayObject *)rows_obj,
                                                   &nrows);
  ...
  for (i = 0; i < nkeys; i++) {
    ...
    if (ls == 0) {
      ...
      if (strncmp(keyname, "COMMENT", tocomp) == 0 ||
        ...
        if (fits_read_key_longstr(self->fits, keyname, &longstr, comment,
                                  &status)) {
          Py_XDECREF(list);
          set_ioerr_string_from_status(status, self);
          return NULL;
    ...
    if (is_blank_key) {
      ...
      if (is_hierarch) {
        ...
        // than the checks done here, so if any conversion is done it has
      ...
      if (!is_string_value && 0 == strlen(longstr)) {
        ...
        if (is_string_value) {
          ...
          if (fits_read_key(self->fits, TLONGLONG, keyname, &lval, comment,
                            &status)) {
